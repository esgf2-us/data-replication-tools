<?php
    ...
    $sites = array();
    $destinations = array('ALCF', 'OLCF');
    foreach ($destinations as $destination) {
        $query = 'SELECT dataset,source,requested,completed,status,directories,files,files_transferred,bytes_transferred,faults,rate FROM transfer WHERE uuid!=\'\' and destination=\'' . $destination . '\' and (status=\'ACTIVE\' or status=\'PAUSED\' or status=\'SUCCEEDED\') order by status, requested desc limit 20';
        $out = pg_query($query);
        $transfers = array();
        while ($row = pg_fetch_assoc($out)) {
            ...
            $transfer = array(
                ...
                'files_transferred' => is_null($files) ? '' : (int) $files_transferred,
